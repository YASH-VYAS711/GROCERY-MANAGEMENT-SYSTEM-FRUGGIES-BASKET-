...
connection = pymysql.connect(
    host='localhost',
    user='root',
    password='ROOT123', #put your password here
    database='vege',  #database name here
    ...
    if 'user' in session:
        ...
        if request.method == 'POST':
            ...
            with connection.cursor() as cursor:
                ...
                sql_check_cart = "SELECT * FROM shopping_cart WHERE customer_id = %s"
                cursor.execute(sql_check_cart, (customer_id,))
                cart = cursor.fetchone()

                if not cart:
                    # If the user doesn't have a cart, create one
                    sql_create_cart = "INSERT INTO shopping_cart (customer_id) VALUES (%s)"
                ...
                try:
                    ...
                    sql_add_to_cart = "INSERT INTO cart_items (cart_id, product_id) VALUES (%s, %s)"
    ...
    if 'user' in session:
        ...
        if request.method == 'GET':
            with connection.cursor() as cursor:
                ...
                sql = """
                    ...
                    FROM 
                        ...
                        JOIN shopping_cart c ON cp.cart_id = c.cart_id 
                    WHERE 
                        c.customer_id = %s
                    GROUP BY 
                        cp.cart_id, 
            ...
            with connection.cursor() as cursor:
                ...
                sql = "DELETE FROM cart_items WHERE cart_id = (SELECT cart_id FROM shopping_cart WHERE customer_id = %s) AND product_id = %s"
...
def checkout():
    if 'user' in session:
        ...
        if request.method == 'POST':
            ...
            with connection.cursor() as cursor:
                sql_delete_cart = "DELETE FROM cart_items WHERE cart_id IN (SELECT cart_id FROM shopping_cart WHERE customer_id = %s)"
